...
export const injectUnfollowUserMutation = (
  { injector }: { injector?: Injector } = {},
  props: { newData: SubscriptionInterface }
) => {
  injector = assertInjectorFn(injectUnfollowUserMutation, injector);
  return runInInjectionContext(injector, () => {
    ...
    return injectMutation((client: QueryClient) => ({
      mutationFn: () =>
        lastValueFrom(subscriptionService.unfollow(props.newData)),
      retry: false,
