...
export const injectDeleteProfileAvatarMutation = ({
  injector,
}: { injector?: Injector } = {}) => {
  injector = assertInjectorFn(injectDeleteProfileAvatarMutation, injector);
  return runInInjectionContext(injector, () => {
    ...
    return injectMutation((client: QueryClient) => ({
      mutationFn: () => lastValueFrom(profileService.deleteProfileAvatar()),
      retry: false,
      onMutate: async () => {
